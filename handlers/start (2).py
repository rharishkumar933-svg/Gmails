...
def apply_constant_reward_to_text(text: str, reward_val: float, exchange_rate: float = 100.0) -> str:
    ...
    if "credited to hold" in text.lower():
        text = re.sub(r'(?i)Password:\s*[^\n]+', "**Password: don't login account back else your account will get ban.**", text)
...
def make_history_keyboard(transactions, total_count, page, exchange_rate, per_page=5) -> InlineKeyboardMarkup:
    ...
    for tx in transactions:
        ...
        if tx_type == "payout":
            ...
            if method == "UPI":
                inr_amount = details.get("inr_amount", abs_usd * exchange_rate)
...
async def handle_user_state_input(client: Client, message: Message):
    ...
    if state == "payout_amount":
        ...
        from config import PAYOUT_UPI_LOG_GROUP_ID, PAYOUT_CRYPTO_LOG_GROUP_ID
        log_group_id = PAYOUT_CRYPTO_LOG_GROUP_ID if method == "crypto" else PAYOUT_UPI_LOG_GROUP_ID
        if log_group_id:
...
🚦 **Status:** Pending Approval"""
            ...
            try:
                ...
                logger.error(f"Failed to send message to payout log group {log_group_id}: {e}")
...
@Client.on_message(filters.text & filters.private & ~filters.command(["start", "admin", "check", "addbalance", "removebalance", "addholdbalance", "removeholdbalance", "waittime", "checkholds", "broadcast", "stats", "user", "payout", "commission", "setup", "minimumpayout", "admincommands", "login", "margin", "setmargin", "reset", "addworker", "workers", "removeworker"]))
async def placeholder_buttons_handler(client: Client, message: Message):
    ...
    if text == "➕ New Account":
        ...
        if latest_reg and latest_reg.get("status") not in ["completed", "cancelled", "rejected", "expired"]:
...
![3️⃣](tg://emoji?id=6035205087045884098) Submit the account details back to the bot and get paid instantly! ![🚀](tg://emoji?id=6021813997492246416)"""
        ...
        referred_users = await mongo.db["users"].find({"referred_by": message.from_user.id}).to_list(length=None)
        referred_user_ids = [u["user_id"] for u in referred_users]
        
        total_registered = 0
        total_approved = 0
        if referred_user_ids:
            total_registered = await mongo.db["transactions"].count_documents({
                "user_id": {"$in": referred_user_ids},
                "status": "hold",
                "registration_id": {"$exists": True}
            })
            total_approved = await mongo.db["transactions"].count_documents({
                "user_id": {"$in": referred_user_ids},
                "status": "credited",
                "registration_id": {"$exists": True}
            })
        ...
        ref_text = (
            ...
            f"📈 **Total Registered (Hold) Accounts by Referrals (This can be decreased if user change credential details):** `{total_registered}`\n"
            f"✅ **Total Approved Accounts by Referrals:** `{total_approved}`\n\n"
            f"⚠️ **Note:** Referral money is not paid only paid when someone create account with your refferal and you will earn amount per account creation."
        )
...
![📅](tg://emoji?id=6023880246128810031) **Date:** `{created_str}`"""
            ...
            details = tx.get("details", {})
            ref_user_id = details.get("referred_user_id", "Unknown")
            text = f"""💸 **Referral Commission Details**
            
🚦 **Status:** {status_str}
![💰](tg://emoji?id=6030558512252197022) **Amount:** {amount_str}
👤 **Action By User ID:** `{ref_user_id}`
...
async def payout_approve_callback(client: Client, callback: CallbackQuery):
    ...
    is_w = (admin_id == OWNER_ID 
            or await mongo.is_admin(admin_id) 
            or await mongo.db["workers"].find_one({"user_id": admin_id}) is not None)
    if not is_w:
        await callback.answer("❌ You are not authorized to perform this action (requires worker or admin).", show_alert=True)
        return
    ...
    if success:
        ...
        from datetime import datetime, timedelta
        approved_at_ist = datetime.utcnow() + timedelta(hours=5, minutes=30)
        time_str = approved_at_ist.strftime("%Y-%m-%d %I:%M:%S %p")
        ...
        # Send action log to the main log group (PAYOUT_LOG_GROUP_ID)
        from config import PAYOUT_LOG_GROUP_ID
        if PAYOUT_LOG_GROUP_ID:
            action_text = (
                f"👷 **Worker Action Log**\n\n"
                f"👤 **Worker:** {admin_mention} (ID: `{admin_id}`)\n"
                f"⚡ **Action:** Approved Payout ✅\n"
                f"💰 **Amount:** `{amount_str}`\n"
                f"👤 **User:** `{target_user_id}`\n"
                f"🕒 **Time:** `{time_str} IST`"
            )
            try:
                await client.send_message(PAYOUT_LOG_GROUP_ID, action_text)
            except Exception as e:
                logger.error(f"Failed to send worker action log to main group: {e}")
...
async def payout_reject_callback(client: Client, callback: CallbackQuery):
    ...
    if success:
        ...
        admin_mention = callback.from_user.mention
        from datetime import datetime, timedelta
        rejected_at_ist = datetime.utcnow() + timedelta(hours=5, minutes=30)
        time_str = rejected_at_ist.strftime("%Y-%m-%d %I:%M:%S %p")
        original_text = callback.message.text
