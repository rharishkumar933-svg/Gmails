...
async def group_processor_stats_command(client: Client, message: Message):
    ...
    # 2. Total INR Value (Only relevant for INR group or we display it contextually)
    inr_match = dict(match_filter)
    inr_match["details.inr_amount"] = {"$exists": True}
    pipeline_inr = [
        {"$match": inr_match},
    ...
    reset_str = f" *(Since last reset: {(reset_at + timedelta(hours=5, minutes=30)).strftime('%Y-%m-%d %I:%M %p IST')})*" if reset_at else ""
    ...
    if is_upi_group:
        stats_text += f"• **Total INR Processed:** `{total_inr:.2f} INR`\n"
    else:
        stats_text += (
            f"• **Total USD Processed:** `{total_usd:.3f}$`\n"
            f"• **Total INR Processed:** `{total_inr:.2f} INR`\n"
        )
