...
class UserbotManager:
        ...
        if "credited to hold" in text and "transferred" in text:
            try:
                ...
                if not existing_tx:
                    ...
                    if days_match:
                        try:
                            hold_days = max(int(days_match.group(1)), 10)
                        except ValueError:
                            pass
    ...
    async def check_pending_hold_accounts(self, save_raw_files=False) -> dict:
        ...
        Queries all 'hold' transactions in the database from the last 6 days.
        """
        logger.info("Starting hold accounts verification...")
        now = datetime.utcnow()
        cutoff_date = now - timedelta(days=6)
        
        # Get all transactions that are in 'hold' status and created in the last 6 days
        ...
        if not hold_txs:
            logger.info("No pending hold transactions found in the last 6 days.")
            return {}
        ...
        for i, match in enumerate(matches):
            ...
            if date_match:
                ...
                try:
                    ...
                    if (now - created_dt).days > 6:
                        logger.info(f"Account {email} created on {created_dt} is older than 6 days. Stopping verification.")
                        stop_checking = True
                        break
                except Exception as ex:
